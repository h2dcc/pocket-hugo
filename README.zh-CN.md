...
PocketHugo 是一个浏览器优先的 Hugo 发布工具，适用于把内容托管在 GitHub 的用户，并兼顾电脑、平板与手机使用。
...
PocketHugo 核心目标是保留 Hugo 原生 page bundle 结构，同时把浏览器中的写作和发布流程简化：

- 可在电脑、平板或手机浏览器中写作和改稿
- `index.md + 图片` 保持同目录，结构清晰
- 上传时即可压缩/转换/自动命名图片，减少网站仓库存储和构建负担
- 一次发布即可批量提交本次改动文件
- 不依赖单独的在线数据库保存你的文章内容
...
一路试错、记录、重构到现在，PocketHugo 才算把“浏览器写作 + Hugo 原生结构 + 稳定发布”这条链路真正打通。
...
- 当请求域名命中 `LANDING_PAGE_HOSTS` 时，根路径 `/` 显示项目介绍页。
- `/app` 永远直接进入应用。
- 如果当前域名不在 `LANDING_PAGE_HOSTS` 中，则根路径 `/` 直接进入应用。
- 这样可以保证其他用户自部署时默认仍然是“打开即用”，而你的主域名可以单独承担展示页角色。
